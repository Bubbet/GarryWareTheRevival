...
local function Uncover(p)
	...
	if num>0 then
		...
		textent:SetEntityInteger(num)

		local c = Colors[num] or Colors[8]

		timer.Simple(0.1, function(t)
			GAMEMODE:SendEntityTextColor(t , textent , c[1], c[2], c[3], 255 )
		end, textent)
	end
end
...
function WARE:Initialize()
	...
	GAMEMODE:DrawInstructions("Prepare for some minesweeping!")
...
function WARE:StartAction()
	GAMEMODE:DrawInstructions("Make the most safe moves, don't get blown up!")
...
function WARE:EndAction()
	...
	if self.NumMines==remaining then
		...
		if table.Count(self.playersCleared) > 1 then table.sort(self.playersCleared, function(a, b) return a.broke > b.broke end) end
		local numWinners = math.Clamp(math.ceil( team.NumPlayers(TEAM_HUMANS) * 0.4) , 1, 64)
		local addedWins = 0
		for k, v in ipairs(self.playersCleared) do
			if addedWins < numWinners then
				--print(v.ply, 'won')
				v.ply:SetAchievedNoLock( true )
				addedWins = addedWins + 1
			end
		end
		...
		--[[
		for i = 1, math.Clamp(math.ceil( team.NumPlayers(TEAM_HUMANS) * 0.4) , 1, 64) do
			local ply = self.playersCleared[i].ply
			if not self.Losers[ply] then
				print(ply, 'won')
...
function WARE:EntityTakeDamage(ent, tbl)

	att = tbl:GetAttacker()
	amount = tbl:GetDamage()
	...
	if ent.Mine then
		...
		table.remove(self.playersCleared, att:EntIndex())
		...
		timer.Simple(3,function() if ent:IsValid() and ent.Mine then ent:SetColor(255,255,255,255) end end)
		...
		if fail then
			...
			timer.Simple(3, function() GAMEMODE:EndGame() end)
			--if GAMEMODE.NextgameEnd>CurTime()+3 then
			--GAMEMODE:SetNextGameEnd(CurTime()+3)
			--end
		end
	else
		local ind = att:EntIndex()
		self.playersCleared[ind] = self.playersCleared[ind] or {ply = att, broke = 0}--(self.playersCleared[att] or 0) + 1
		self.playersCleared[ind].broke = self.playersCleared[ind].broke + 1
	end
end
